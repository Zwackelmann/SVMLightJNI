...
/* kernel_cache: Initialized Cache of size totdoc, if using a kernel. NULL if linear.*/
 ...
 */
void svm_learn_classification(DOC **docs, double *category, long int
		totdoc, long int totwords, LEARN_PARM *learn_parm, 
...
void svm_learn_regression(DOC **docs, double *value, long int totdoc, 
		long int totwords, LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm, 
...
/* alpha:       Start values for the alpha variables or NULL pointer. 
 * The new alpha values are returned after optimization if not NULL. Array must be of size totdoc. 
 */
...
/* kernel_parm: Kernel paramenters */
/* kernel_cache: Initialized/partly filled Cache, if using a kernel. NULL if linear. */
...
long optimize_to_convergence(DOC **docs, long int *label, long int totdoc, 
		...
		long int *inconsistent, long int *unlabeled, double *a, double *lin, 
		double *c, TIMING *timing_profile, double *maxdiff, long int heldout, 
		long int retrain) {
...
long optimize_to_convergence_sharedslack(DOC **docs, long int *label, 
		long int totdoc, long int totwords, LEARN_PARM *learn_parm, 
		KERNEL_PARM *kernel_parm, KERNEL_CACHE *kernel_cache, 
		SHRINK_STATE *shrink_state, MODEL *model, double *a, 
		double *lin, double *c, TIMING *timing_profile, double *maxdiff) {
...
double compute_objective_function(double *a, double *lin, double *c, 
		double eps, long int *label, long int *active2dnum) {
	long i,ii;
	double criterion;
...
void optimize_svm(DOC **docs, long int *label, long int *unlabeled, 
		long int *exclude_from_eq_const, double eq_target, long int *chosen, 
		long int *active2dnum, MODEL *model, long int totdoc, 
		long int *working2dnum, long int varnum, double *a, double *lin, 
		double *c, LEARN_PARM *learn_parm, CFLOAT *aicache, 
		KERNEL_PARM *kernel_parm, QP *qp, double *epsilon_crit_target) {
	long i;
	double *a_v;
...
void compute_matrices_for_optimization(DOC **docs, long int *label, 
		...
		long int *chosen, long int *active2dnum, long int *key, MODEL *model, 
		double *a, double *lin, double *c, long int varnum, long int totdoc, 
...
long check_optimality_sharedslack(DOC **docs, MODEL *model, long int *label,
		double *a, double *lin, double *c, double *slack, double *alphaslack,
		long int totdoc, LEARN_PARM *learn_parm, double *maxdiff, 
...
void compute_shared_slacks(DOC **docs, long int *label, 
		double *a, double *lin, double *c, long int *active2dnum,
		LEARN_PARM *learn_parm, double *slack, double *alphaslack) {
	long jj,i;
	double dist,target;
...
long identify_inconsistent(double *a, long int *label, 
		...
		LEARN_PARM *learn_parm, long int *inconsistentnum, 
...
long identify_misclassified(double *lin, long int *label, 
		long int *unlabeled, long int totdoc, MODEL *model, 
...
long incorporate_unlabeled_examples(MODEL *model, long int *label, 
		...
		long int *select, long int *key, long int transductcycle, 
...
long select_next_qp_subproblem_grad(long int *label, 
		long int *unlabeled, double *a, double *lin, 
		double *c, long int totdoc, long int qp_size, 
		LEARN_PARM *learn_parm, long int *inconsistent, 
		long int *active2dnum, long int *working2dnum, 
		double *selcrit, long int *select, 
		KERNEL_CACHE *kernel_cache, long int cache_only,
...
long select_next_qp_slackset(DOC **docs, long int *label, 
		double *a, double *lin, double *slack, 
		double *alphaslack, double *c, LEARN_PARM *learn_parm, 
...
long shrink_problem(DOC **docs,
		LEARN_PARM *learn_parm, SHRINK_STATE *shrink_state, 
		KERNEL_PARM *kernel_parm, long int *active2dnum, 
		long int *last_suboptimal_at, long int iteration, 
		long int totdoc, long int minshrink, 
...
/* Make all variables active again which had been removed by shrinking. */
/* Computes lin for those variables from scratch. */
void reactivate_inactive_examples(long int *label, 
		long int *unlabeled, double *a, SHRINK_STATE *shrink_state, 
...
/* Get a free cache entry. In case cache is full, the lru element is removed. */
...
double estimate_r_delta(DOC **docs, long int totdoc, KERNEL_PARM *kernel_parm) {
	...
	maxxlen=0;
	for(i=0;i<totdoc;i++) {
		xlen=sqrt(kernel(kernel_parm,docs[i],docs[i])-2*kernel(kernel_parm,docs[i],nulldoc)+kernel(kernel_parm,nulldoc,nulldoc));
		if(xlen>maxxlen) {
			maxxlen=xlen;
		}
	}
