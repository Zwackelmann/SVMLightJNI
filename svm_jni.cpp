...
JavaParamIDs* GetJParamIDs(JNIEnv * env, jobjectArray* tdata) {
	JavaParamIDs *ids = (JavaParamIDs*)my_malloc(sizeof(struct javaparamids));

	// Finde und Bestimme Klassentyp von SVMLightModel
	ids->SVMLightModelCls = env->FindClass("jnisvmlight/SVMLightModel");
	...
	ids->ID_string_format = env->GetFieldID(ids->SVMLightModelCls, "m_format", "Ljava/lang/String;");
	ids->ID_long_kType = env->GetFieldID(ids->SVMLightModelCls, "m_kType", "J");
	ids->ID_long_dParam = env->GetFieldID(ids->SVMLightModelCls, "m_dParam", "J");
	ids->ID_double_gParam = env->GetFieldID(ids->SVMLightModelCls, "m_gParam", "D");
	ids->ID_double_sParam = env->GetFieldID(ids->SVMLightModelCls, "m_sParam", "D");
	ids->ID_double_rParam = env->GetFieldID(ids->SVMLightModelCls, "m_rParam", "D");
	ids->ID_string_uParam = env->GetFieldID(ids->SVMLightModelCls, "m_uParam", "Ljava/lang/String;");
	ids->ID_long_highFeatIdx = env->GetFieldID(ids->SVMLightModelCls, "m_highFeatIdx", "J");
	ids->ID_long_trainDocs = env->GetFieldID(ids->SVMLightModelCls, "m_trainDocs", "J");
	ids->ID_long_numSupVecs = env->GetFieldID(ids->SVMLightModelCls, "m_numSupVecs", "J");
	ids->ID_double_threshold = env->GetFieldID(ids->SVMLightModelCls, "m_threshold", "D");
	ids->ID_doubleArray_linWeights = env->GetFieldID(ids->SVMLightModelCls, "m_linWeights", "[D");
	ids->ID_labeledFeatureVectorArray_docs = env->GetFieldID(ids->SVMLightModelCls, "m_docs", "[Ljnisvmlight/LabeledFeatureVector;");
	...
	ids->ConstructorID_SVMLightModelCls = 
		env->GetMethodID(ids->SVMLightModelCls, 
		"<init>", 
	...
	ids->tDataSize = env->GetArrayLength(*tdata);
	...
	jobject traindoc = env->GetObjectArrayElement(*tdata, 0);	
	if (traindoc == NULL) {
		perror("\ntraining document is null!\n");
	}

	// Klassentyp des Trainingsdokuments bestimmen
	ids->tDataCls = env->GetObjectClass(traindoc);
	if (ids->tDataCls == 0) {
	...
	ids->ID_double_label = env->GetFieldID(ids->tDataCls, "m_label", "D");
	ids->ID_double_factor = env->GetFieldID(ids->tDataCls, "m_factor", "D");
	ids->ID_intArray_dimensions = env->GetFieldID(ids->tDataCls, "m_dims", "[I");
	ids->ID_doubleArray_values = env->GetFieldID(ids->tDataCls, "m_vals", "[D");
	// ids->MemVarID_size = (*env)->GetFieldID(ids->tDataCls, "m_size", "I");
	...
	ids->ConstructorID_tDataCls = env->GetMethodID(ids->tDataCls, "<init>", "()V");
...
JTrainParams* GetJTrainParamIDs(JNIEnv * env, jobject* tparam) {
	...
	JTrainParams *tids = (JTrainParams*)(my_malloc(sizeof(struct jtrainparams)));

	tids->env=env;

	jclass tparamCls = env->FindClass("jnisvmlight/TrainingParameters");
	if (tparamCls == 0) {
		perror("Can't determine the class of 'TrainingParameters': perror()");
		exit(1);
	}

	tids->ID_LearnParam_lp = env->GetFieldID(tparamCls, "m_lp", "Ljnisvmlight/LearnParam;");
	tids->ID_KernelParam_kp = env->GetFieldID(tparamCls, "m_kp", "Ljnisvmlight/KernelParam;");
	...
	tids->lp = env->GetObjectField(*tparam, tids->ID_LearnParam_lp);	
	tids->kp = env->GetObjectField(*tparam, tids->ID_KernelParam_kp);	
	...
	jclass lpCls = env->GetObjectClass(tids->lp);
	jclass kpCls = env->GetObjectClass(tids->kp);
	if ((lpCls && kpCls) == 0) {
		perror("Can't determine the class of 'm_lp' or 'm_kp': perror()");
		exit(1);
	}

	tids->ID_int_verbosity = env->GetFieldID(lpCls, "verbosity", "I");
	tids->ID_long_type = env->GetFieldID(lpCls, "type", "J");
	tids->ID_double_svm_c = env->GetFieldID(lpCls, "svm_c", "D");
	tids->ID_double_eps = env->GetFieldID(lpCls, "eps", "D");
	tids->ID_double_svm_costratio = env->GetFieldID(lpCls, "svm_costratio", "D");
	tids->ID_double_transduction_posratio = env->GetFieldID(lpCls, "transduction_posratio", "D");
	tids->ID_long_biased_hyperplane = env->GetFieldID(lpCls, "biased_hyperplane", "J");
	tids->ID_long_sharedslack = env->GetFieldID(lpCls, "sharedslack", "J");
	tids->ID_long_svm_maxqpsize = env->GetFieldID(lpCls, "svm_maxqpsize", "J");
	tids->ID_long_svm_newvarsinqp = env->GetFieldID(lpCls, "svm_newvarsinqp", "J");
	tids->ID_long_kernel_cache_size = env->GetFieldID(lpCls, "kernel_cache_size", "J");
	tids->ID_double_epsilon_crit = env->GetFieldID(lpCls, "epsilon_crit", "D");
	tids->ID_double_epsilon_shrink = env->GetFieldID(lpCls, "epsilon_shrink", "D");
	tids->ID_long_svm_iter_to_shrink = env->GetFieldID(lpCls, "svm_iter_to_shrink", "J");
	tids->ID_long_maxiter = env->GetFieldID(lpCls, "maxiter", "J");
	tids->ID_long_remove_inconsistent = env->GetFieldID(lpCls, "remove_inconsistent", "J");
	tids->ID_long_skip_final_opt_check = env->GetFieldID(lpCls, "skip_final_opt_check", "J");
	tids->ID_long_compute_loo = env->GetFieldID(lpCls, "compute_loo", "J");
	tids->ID_double_rho = env->GetFieldID(lpCls, "rho", "D");
	tids->ID_long_xa_depth = env->GetFieldID(lpCls, "xa_depth", "J");
	tids->ID_string_predfile = env->GetFieldID(lpCls, "predfile", "Ljava/lang/String;");
	tids->ID_string_alphafile = env->GetFieldID(lpCls, "alphafile", "Ljava/lang/String;");
	tids->ID_double_epsilon_const = env->GetFieldID(lpCls, "epsilon_const", "D");
	tids->ID_double_epsilon_a = env->GetFieldID(lpCls, "epsilon_a", "D");
	tids->ID_double_opt_precision = env->GetFieldID(lpCls, "opt_precision", "D");
	tids->ID_long_svm_c_steps = env->GetFieldID(lpCls, "svm_c_steps", "J");
	tids->ID_double_svm_c_factor = env->GetFieldID(lpCls, "svm_c_factor", "D");
	tids->ID_double_svm_costratio_unlab = env->GetFieldID(lpCls, "svm_costratio_unlab", "D");
	tids->ID_double_svm_unlabbound = env->GetFieldID(lpCls, "svm_unlabbound", "D");
	tids->ID_double_svm_cost = env->GetFieldID(lpCls, "svm_cost", "D");
	tids->ID_long_totwords = env->GetFieldID(lpCls, "totwords", "J");
	...
	tids->ID_long_kernel_type = env->GetFieldID(kpCls, "kernel_type", "J");
	tids->ID_long_poly_degree = env->GetFieldID(kpCls, "poly_degree", "J");
	tids->ID_double_rbf_gamma = env->GetFieldID(kpCls, "rbf_gamma", "D");
	tids->ID_double_coef_lin = env->GetFieldID(kpCls, "coef_lin", "D");
	tids->ID_double_coef_const = env->GetFieldID(kpCls, "coef_const", "D");
	tids->ID_string_custom = env->GetFieldID(kpCls, "custom", "Ljava/lang/String;");
	...
	jfieldID argcID = env->GetFieldID(lpCls, "argc", "I");
	jfieldID argvID = env->GetFieldID(lpCls, "argv", "[Ljava/lang/String;");
	...
	tids->argc = env->GetIntField(tids->lp,argcID);

	jobjectArray sfield = (jobjectArray)(env->GetObjectField(tids->lp,argvID));
	if (tids->argc > 0) {
		tids->argv = (char**) my_malloc(sizeof(char*) * tids->argc);
		int j;
		for (j=0;j<tids->argc;j++) {
			jstring jstr = (jstring)(env->GetObjectArrayElement(sfield, j));
...
void SVMparmInit(KERNEL_CACHE* kernel_cache,LEARN_PARM* learn_parm,KERNEL_PARM* kernel_parm, MODEL* model, JTrainParams* tparm) {
	...
	verbosity = env->GetIntField(tparm->lp,tparm->ID_int_verbosity);

	learn_parm->type = (long)(env->GetLongField(tparm->lp,tparm->ID_long_type));
	/* learn_parm->svm_c=0.0; */
	learn_parm->svm_c = env->GetDoubleField(tparm->lp,tparm->ID_double_svm_c);                
	/* learn_parm->eps=0.1; */
	learn_parm->eps = env->GetDoubleField(tparm->lp,tparm->ID_double_eps);                  
	/* learn_parm->svm_costratio=1.0; */
	learn_parm->svm_costratio = env->GetDoubleField(tparm->lp,tparm->ID_double_svm_costratio);        
	/* learn_parm->transduction_posratio=-1.0; */
	learn_parm->transduction_posratio = env->GetDoubleField(tparm->lp,tparm->ID_double_transduction_posratio);
	/* learn_parm->biased_hyperplane=1; */
	learn_parm->biased_hyperplane = (long)(env->GetLongField(tparm->lp,tparm->ID_long_biased_hyperplane));    
	/* learn_parm->sharedslack=0; */
	learn_parm->sharedslack = (long)(env->GetLongField(tparm->lp,tparm->ID_long_sharedslack));          
	/* learn_parm->svm_maxqpsize=10; */
	learn_parm->svm_maxqpsize = (long)(env->GetLongField(tparm->lp,tparm->ID_long_svm_maxqpsize));        
	/* learn_parm->svm_newvarsinqp=0; */
	learn_parm->svm_newvarsinqp = (long)(env->GetLongField(tparm->lp,tparm->ID_long_svm_newvarsinqp));      
	/* learn_parm->kernel_cache_size=40; */
	learn_parm->kernel_cache_size = (long)(env->GetLongField(tparm->lp,tparm->ID_long_kernel_cache_size));    
	/* learn_parm->epsilon_crit=0.001; */
	learn_parm->epsilon_crit = env->GetDoubleField(tparm->lp,tparm->ID_double_epsilon_crit);         

	learn_parm->epsilon_shrink = env->GetDoubleField(tparm->lp,tparm->ID_double_epsilon_shrink);       
	/* learn_parm->svm_iter_to_shrink=-9999; */
	learn_parm->svm_iter_to_shrink = (long)(env->GetLongField(tparm->lp,tparm->ID_long_svm_iter_to_shrink));   
	/* learn_parm->maxiter=100000; */
	learn_parm->maxiter = (long)(env->GetLongField(tparm->lp,tparm->ID_long_maxiter));              
	/* learn_parm->remove_inconsistent=0; */
	learn_parm->remove_inconsistent = (long)(env->GetLongField(tparm->lp,tparm->ID_long_remove_inconsistent));  
	/* learn_parm->skip_final_opt_check=0; */
	learn_parm->skip_final_opt_check = (long)(env->GetLongField(tparm->lp,tparm->ID_long_skip_final_opt_check)); 
	/* learn_parm->compute_loo=0; */
	learn_parm->compute_loo = (long)(env->GetLongField(tparm->lp,tparm->ID_long_compute_loo));          
	/* learn_parm->rho=1.0; */
	learn_parm->rho = env->GetDoubleField(tparm->lp,tparm->ID_double_rho);                  
	/* learn_parm->xa_depth=0; */
	learn_parm->xa_depth = (long)(env->GetLongField(tparm->lp,tparm->ID_long_xa_depth));             


	/* strcpy (learn_parm->predfile, "trans_predictions"); */
	test = (jstring)(env->GetObjectField(tparm->lp, tparm->ID_string_predfile));
	str = env->GetStringUTFChars(test, 0 );
	strcpy (learn_parm->predfile, str);
	env->ReleaseStringUTFChars(test,str);
	/* strcpy (learn_parm->alphafile, ""); */
	test = (jstring)(env->GetObjectField(tparm->lp, tparm->ID_string_alphafile));
	str = env->GetStringUTFChars(test, 0 );
	strcpy (learn_parm->alphafile, str);
	env->ReleaseStringUTFChars(test,str);


	learn_parm->epsilon_const = env->GetDoubleField(tparm->lp,tparm->ID_double_epsilon_const);        
	/* learn_parm->epsilon_a=1E-15; */
	learn_parm->epsilon_a = env->GetDoubleField(tparm->lp,tparm->ID_double_epsilon_a);            

	learn_parm->opt_precision = env->GetDoubleField(tparm->lp,tparm->ID_double_opt_precision);        

	learn_parm->svm_c_steps = (long)(env->GetLongField(tparm->lp,tparm->ID_long_svm_c_steps));          

	learn_parm->svm_c_factor = env->GetDoubleField(tparm->lp,tparm->ID_double_svm_c_factor);         
	/* learn_parm->svm_costratio_unlab=1.0; */
	learn_parm->svm_costratio_unlab = env->GetDoubleField(tparm->lp,tparm->ID_double_svm_costratio_unlab);
	/* learn_parm->svm_unlabbound=1E-5; */
	learn_parm->svm_unlabbound = env->GetDoubleField(tparm->lp,tparm->ID_double_svm_unlabbound);

	learn_parm->svm_cost = (double *) my_malloc(sizeof(double));            
	*(learn_parm->svm_cost) =	(double) env->GetDoubleField(tparm->lp,tparm->ID_double_svm_cost);

	learn_parm->totwords = (long)(env->GetLongField(tparm->lp,tparm->ID_long_svm_c_steps));             

	/* kernel_parm->kernel_type=0; */
	kernel_parm->kernel_type = (long)(env->GetLongField(tparm->kp,tparm->ID_long_kernel_type)); 
	/* kernel_parm->poly_degree=3; */
	kernel_parm->poly_degree = (long)(env->GetLongField(tparm->kp,tparm->ID_long_poly_degree));
	/* kernel_parm->rbf_gamma=1.0; */
	kernel_parm->rbf_gamma = env->GetDoubleField(tparm->kp,tparm->ID_double_rbf_gamma);
	/* kernel_parm->coef_lin=1; */
	kernel_parm->coef_lin = env->GetDoubleField(tparm->kp,tparm->ID_double_coef_lin);

	/* kernel_parm->coef_const=1; */
	kernel_parm->coef_const = env->GetDoubleField(tparm->kp,tparm->ID_double_coef_const);

	/* strcpy(kernel_parm->custom,"empty"); */
	test = (jstring)(env->GetObjectField(tparm->kp, tparm->ID_string_custom));
	str = env->GetStringUTFChars(test, 0 );
...
JNIEXPORT jdouble JNICALL Java_jnisvmlight_SVMLightInterface_classifyNative(JNIEnv * env, jobject jo1, jobject testdoc) {
	...
	jclass classid = env->GetObjectClass(testdoc);

	jintArray dim = (jintArray)(env->GetObjectField(testdoc, env->GetFieldID(classid, "m_dims", "[I"))); 
	jdoubleArray val = (jdoubleArray)(env->GetObjectField(testdoc, env->GetFieldID(classid, "m_vals", "[D")));

	jsize dimLen = env->GetArrayLength(dim);
	jsize valLen = env->GetArrayLength(val);

	jint *dimEl = env->GetIntArrayElements(dim, 0);
	jdouble *valEl = env->GetDoubleArrayElements(val, 0);
	...
	env->ReleaseIntArrayElements(dim,dimEl,0);
	env->ReleaseDoubleArrayElements(val,valEl,0);
	...
	(JNIEnv * env, jobject obj, jobjectArray tdata, jobject tparm)
{
	...
	SVMparmInit(kernel_cache,&learn_parm,&kernel_parm,model,targs);
	...
	if (*ndocuments > 0) {
		...
		if(learn_parm.type == CLASSIFICATION) {
			...
			svm_learn_ranking(docs,target,*totdoc,*totwords,&learn_parm,&kernel_parm,&kernel_cache,model);
		}
		...
		model->index = (long *)my_malloc(sizeof(long)*2);
...
jobject buildModelData(JNIEnv *env, jobject obj, MODEL* model,JavaParamIDs* ids) {
	...
	if (NUM_DOCS > 1) {
		...
		doks = env->NewObjectArray((jsize) NUM_DOCS-1, ids->tDataCls, NULL);	
		if (doks == 0) {
		...
		for (u=1; u<NUM_DOCS; u++) {
			...
			jobject Data =env->NewObject(ids->tDataCls, ids->ConstructorID_tDataCls);
			...
			env->SetDoubleField(Data, ids->ID_double_label, label);
			...
			//env->SetIntField(Data, ids->MemVarID_size, size);
			...
			jintArray dim = env->NewIntArray((jsize) NUM_FEAT);
			jdoubleArray val = env->NewDoubleArray((jsize) NUM_FEAT);
			...
			for (j=0;j<NUM_FEAT;j++) {
				...
				int wnum = (int)((v->words[j]).wnum);
				double weight = (double) ((v->words[j]).weight);

				env->SetIntArrayRegion(dim, (jsize) j, (jsize) 1, (jint*) &wnum);
			    env->SetDoubleArrayRegion(val, (jsize) j, (jsize) 1, (jdouble*) &weight);
			}

			// Lege Java-Arrays in Objekt des Typs jnisvmlight/LabeledFeatureVector ab
			env->SetObjectField(Data, ids->ID_intArray_dimensions, dim);
			env->SetObjectField(Data, ids->ID_doubleArray_values, val);
			// lege Objekt vom Type jnisvmlight/LabeledFeatureVector in jnisvmlight/LabeledFeatureVector-Array ab (an Position u-1)
			env->SetObjectArrayElement(doks, (jsize) u-1, Data); 
		...
		//  	env->SetObjectField(SVMLightModel, ids->ID_labeledFeatureVectorArray_docs, doks);
	} else {
		fprintf(stderr,"-------------------------------------------------------------------------------\nThe number of suppert vecors (model->sv_num: %ld) is less than 2!\nThere must be at least 2 support vectors. Model can't be built.\n",model->sv_num);
		// 	env->SetObjectField(SVMLightModel, ids->ID_labeledFeatureVectorArray_docs, NULL);
	...
	jobject SVMLightModel =env->NewObject(ids->SVMLightModelCls, ids->ConstructorID_SVMLightModelCls, 
		env->NewStringUTF(dummy),
		...
		env->NewStringUTF((model->kernel_parm.custom)),
	...
	if ( (model->kernel_parm.kernel_type) == 0 ) {
		jdoubleArray val = env->NewDoubleArray((jsize) model->totwords +1);
		for (j=0;j<model->totwords +1;j++) {
			env->SetDoubleArrayRegion(val, (jsize) j, (jsize) 1, (jdouble*) &(model->lin_weights)[j]);
		}
		env->SetObjectField(SVMLightModel, ids->ID_doubleArray_linWeights, val);
	} else {
		double nullpointer = 0;
		env->SetObjectField(SVMLightModel, ids->ID_doubleArray_linWeights, (jobject)&nullpointer);
	}

	// Belege restliche Membervariablen des Objekts vom Typ SVMLightModel mit zugehoerigen Werten
	//env->SetObjectField(SVMLightModel, ids->ID_string_format, (env->NewStringUTF(dummy)));
	free(dummy);

	//env->SetLongField(SVMLightModel, ids->ID_long_kType, (jlong) (model->kernel_parm.kernel_type));
	//env->SetLongField(SVMLightModel, ids->ID_long_dParam, (jlong) (model->kernel_parm.poly_degree));
	//env->SetDoubleField(SVMLightModel, ids->ID_double_gParam, (jdouble) (model->kernel_parm.rbf_gamma));
	//env->SetDoubleField(SVMLightModel, ids->ID_double_sParam, (jdouble)  (model->kernel_parm.coef_lin));
	//(*env)->SetDoubleField(SVMLightModel, ids->ID_double_rParam, (jdouble) (model->kernel_parm.coef_const) );
	//(*env)->SetObjectField(SVMLightModel, ids->ID_string_uParam, (*env)->NewStringUTF((model->kernel_parm.custom)));
	//(*env)->SetLongField(SVMLightModel, ids->ID_long_highFeatIdx, (jlong) (model->totwords) );
	//(*env)->SetLongField(SVMLightModel, ids->ID_long_trainDocs, (jlong) (model->totdoc));
	//(*env)->SetLongField(SVMLightModel, ids->ID_long_numSupVecs, (jlong) (model->sv_num));
	//(*env)->SetDoubleField(SVMLightModel, ids->ID_double_threshold, (jdouble) (model->b));
...
void createDOCs(JNIEnv * env,JavaParamIDs *JIDs,jobjectArray* tdata,DOC*** docs, double** target, long* totwords, long* totdoc, long* ndocuments) {
	...
	*totdoc = (long) JIDs->tDataSize;
	...
	for (k=0; k<JIDs->tDataSize; k++) {
		...
		jobject traindoc = env->GetObjectArrayElement(*tdata, k);
		if (traindoc == NULL) { 
			if (verbosity>2) {	
				// nexus: debugging ..
		...
		jdouble label = env->GetDoubleField(traindoc, JIDs->ID_double_label); 
		...
		jintArray dim = (jintArray)(env->GetObjectField(traindoc, JIDs->ID_intArray_dimensions)); 
		jdoubleArray val = (jdoubleArray)(env->GetObjectField(traindoc, JIDs->ID_doubleArray_values));
		if ( (dim && val)== 0) {
		...
		jsize valLen = env->GetArrayLength(val);
		if ((dimLen != valLen)) {
			perror("---------------------------------------------------------- arrays are of different size!\n");
			exit(1);
		}
